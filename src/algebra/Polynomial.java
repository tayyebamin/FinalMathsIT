...
/**
 * This class enables to generate objects representing polynomials with real
 * coefficients. A polynomial with real coefficients has the form
 * <p style="text-align:center">
 * <i>p</i>(<i>x</i>) &nbsp; = &nbsp; <i>a</i><sub>0</sub> +
 * <i>a</i><sub>1</sub> <i>x</i> + <i>a</i><sub>2</sub> <i>x</i><sup>2</sup> +
 * &nbsp; ... &nbsp; + <i>a<sub>n</sub></i> <i>x<sup>n</sup></i>
 * </p>
 * where <i>a</i><sub>0</sub>, <i>a</i><sub>1</sub>, ..., <i>a<sub>n</sub></i>
 * &#x2208; <span style="font-size:large;">&#x211D;</span>, and
 * <i>a<sub>n</sub></i> &#x01C2; 0. Then <i>n</i> is called the <i>degree</i> of
 * the polynomial. Internally, a polynomial is represented by a sorted map,
 * where the key represents the unique exponent and the value the respective
 * coefficient, i.e., is given by the map
 * <p style="text-align:center">
 * [&lt;<i>n</i>,<i>a<sub>n</sub></i>&gt;, ..., &lt;0,<i>a</i><sub>0</sub>&gt;]
 * </p>
 * The default comparator for the exponents, i.e., the keys of this TreeMap, is
 * {@link ExponentComparator}, with a <i>descending</i> order. The simplest way
 * to create a polynomial is given by the following code snippet:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Here the order of put instructions is arbitrary. This object then represents
 * the polynomial
 * <p style="text-align:center">
 * <i>p</i>(<i>x</i>) &nbsp; = &nbsp; <i>x</i><sup>1023</sup> -
 * 3.5<i>x</i><sup>2</sup> + 3<i>x</i> - 1
 * </p>
 * Additionally, this class provides some easy to use and comparably fast static
 * methods for polynomial operations where a polynomial is represented by an
 * array of double values. The principle of this representation is as follows:
 * The element at index <i>i</i> of the array denotes the coefficient
 * <i>a<sub>i</sub></i> of the polynomial, and the length of the array is &gt;=
 * the degree of the polynomial. If the degree of the involved polynomials is
 * not too large, these method are fast.
 * 
 * @see PolynomialZ
...
public class Polynomial extends TreeMap<Integer, Double> {
	...
	/**
	 * Comparably fast method which returns an array representing the polynomial
	 * given by the polynomial product of <i>p</i> and <i>q</i>. The principle
	 * representation is as follows: The element at index <i>i</i> of the array
	 * denotes the coefficient <i>a<sub>i</sub></i> of the polynomial, and the
	 * length of the array is &gt;= the degree of the polynomial. If the degree
	 * of the involved polynomials is not too large, this method is fast and
	 * easy to use. Compare this static method to the object method
	 * {@link #multiply(Polynomial)}.
	 * 
	 * @param p
	 *            array representing the first polynomial
	 * @param q
	 *            array representing the second polyniomial
	 ...
	 * representing the quotient <i>q</i> of the two given polynomials
	 * <i>u/v</i> and the remainder <i>r</i>. The principle of the array
	 * representation is as follows: The element at index <i>i</i> of the array
	 * denotes the coefficient <i>a<sub>i</sub></i> of the polynomial, and the
	 * length of the array is &gt;= the degree of the polynomial. If the degree
	 * of the involved polynomials is not too large, this method is fast and
	 ...
	 * @param v
	 *            array representing the second polyniomial
	 * @return the array {<i>q,r</i>} where <i>q</i> is the quotient polynomial
	 *         <i>u/v</i>, and <i>r</i> is the remainder polynomial such that
	 *         <i>u</i> = <i>qv</i> + <i>r</i>
	...
	public static double[][] divide(double[] u, double[] v) {
		...
		for (k = n - nv; k >= 0; k--) {
			// q_k = r_{nv+k} / v_{nv}:
			q[k] = r[nv + k] / v[nv];
			// System.out.println("### 1.1 k=" + k + ", nv=" + nv + ", r="+r +",
			// q="+q + ", u="+u +", v="+v);
			for (j = nv + k - 1; j >= k; j--) {
				// r_j -= q_k * v_{j-k}:
				r[j] -= q[k] * v[j - k];
				// System.out.println("### 1.2.1 k=" + k + ", j="+j +", nv=" +
				// nv + ", r="+r +", q="+q );
			}
	 ...
	 * The degree is defined as the maximum exponent of the polynomial. For the
	 * principle how an array represents a polynomial, see
	 * {@link #multiply(double[],double[])} By definition, an array consisting
	 * only of zeros represents a polynomial of degree zero.
	 * 
	 * @param p
	...
	/**
	 * Returns a string representation of the given polynomial. For the
	 * principle how an array represents a polynomial, see
	 * {@link #multiply(double[],double[])}
	 * 
	 * @param p
	 *            array representing a polynomial <i>p</i>
	...
	// --- object methods: -----------------------------------------
	/**
	 * Adds the term +<i>a<sub>e</sub> x<sup>e</sup></i> to this polynomial.
	 * Here <i>a<sub>e</sub></i> may be negative, whereas <i>e</i> must be a
	 * non-negative integer.
	 * 
	 * @param exponent
	 *            the exponent <i>e</i> in the term <i>a<sub>e</sub>
	 *            x<sup>e</sup></i>
	 * @param coefficient
	 *            the coefficient <i>a<sub>e</sub></i> in the term
	 *            <i>a<sub>e</sub> x<sup>e</sup></i>
	 * @return the previous coefficient associated with the exponent, or null if
	 *         there did not exist a term with the exponent <i>e</i> in the
	 *         polynomial.
	 */
	@Override
	...
	public Polynomial multiply(Polynomial q) {
		...
		for (i = 0; i <= degP; i++) {
			for (j = 0; j <= degQ; j++) {
				// r_{i+j} += p_i q_j:
				// System.out.println("### 1.1 (i,j)=("+i+","+j+ "), r="+r +",
				// p="+p + ", q="+q);
		...
		/*
		 * for( Iterator e = r.keySet().iterator(); e.hasNext(); ) { //k =
...
//		}
	...
	/**
	 * Divides this polynomial by the given polynomial <i>v</i> and returns an
	 * array {q,r} holding the quotient <i>q</i> as the first entry and the
	 * remainder <i>r</i> as the second entry.
	 * 
	 * @param v
	 *            the polynomial to divide this polynomial
	 * @return the array {<i>q,r</i>} where <i>q</i> is the quotient of this
	 *         polynomial, say <i>u</i>, over <i>v</i>, and <i>r</i> is the
	 *         remainder polynomial such that <i>u</i> = <i>qv</i> + <i>r</i>
	 * @see #divide(double[],double[])
	 */
	public Polynomial[] divide(Polynomial v) {
		...
		for (k = n - nv; k >= 0; k--) {
			...
			if (r.get(nvk) != null) {
				// tmp = ((Double) r.get( nvk ) ).doubleValue() /
				// ((Double)v.get(new Integer(nv))).doubleValue();
				q.put(k, r.get(nvk) / (v.get(nv)));
				// } else {
				// tmp = 0;
			}
			// if ( Math.abs(tmp) > ACCURACY ) q.put( new Integer(k), new
			// Double(tmp) );
			// System.out.println("### 1.2 k=" + k + ", nv=" + nv + ", r="+r +",
			// q="+q + ", u="+u +", v="+v);
			// q.put( k, ( new BigDecimal((BigInteger)r.get(
			// nv.add(k)))).divide( new BigDecimal((BigInteger)v.get(nv)),
	...
	/**
	 * Returns the degree of this polynomial. The degree is defined as the
	 * maximum exponent of the polynomial. Since this polynomial is sorted with
	 * respect to the exponents in descending order, the first key of this map
	 * is the degree. By definition, an empty polynomial has degree zero.
	 * 
	...
	public int deg() {
		// return degree;
		// /* more efficient, since there are usually more puts than polynomial
		// generatings:
	...
	public BigDecimal evaluate(Double value) {
		BigDecimal bd_value;
	...
	public Polynomial plus(Polynomial q) {
		...
		for (Integer i : exponents) {
			if (p.get(i) != null) {
				if (p.get(i).equals(BigInteger.valueOf(q.get(i).longValue()).negate())) {
					// if
					// (p.get(i).equals(BigInteger.valueOf(Long.parseLong(String.valueOf(q.get(i)))).negate())){
					r.remove(i);
				} else {
	...
	public Polynomial givePolynomial(String input) {
		Polynomial p = new Polynomial();
		boolean MinusFlag = false, CoFlag = false;
		Character d='0', temp;
		String co = "", ex = "";
		int exponent;
		double coefficient;
		...
		for (int i = 0; i < input.length(); i++) {
			...
			CoFlag = false;
			d = input.charAt(i);
			while (co == "" || ex == "" || Character.isDigit(d)) {
				d = input.charAt(i);
				i++;
				if (d.equals('-')) {
					MinusFlag = true;
				...
				if (Character.isDigit(d)) {
					if (!CoFlag) {
						if (MinusFlag) { co="-"; MinusFlag=false;}
						co = co + d;
						if (i == input.length() ) {
							break;
						}
						...
						if (MinusFlag) { ex="-"; MinusFlag=false;}
						if (co=="") {
							co="1";
						}
						ex = ex + d;
			...
			i--;
			p.put(Integer.parseInt(ex==""?"0":ex), Double.parseDouble(co));
