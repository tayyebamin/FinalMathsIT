...
public class OrderedSet<E extends Comparable<E>> extends java.util.TreeSet<E> {
   ...
   public OrderedSet<E> minus(SortedSet<E> minuend) {
      OrderedSet<E> s = new OrderedSet<E>();
      for (E x : this) {
         s.add(x);
      }
      s.removeAll(minuend);
   ...
   public static <E extends Comparable<E>> ArrayList<OrderedSet<E>> subsets(SortedSet<E> set, int k) {
      ArrayList<OrderedSet<E>> subsets = new ArrayList<OrderedSet<E>>();
      if (k == 0) {
      ...
      OrderedSet<E> tmpS;
      ArrayList<E> s = new ArrayList<E>(set);
      if (k == 1) {
         for (E i : s) {
            tmpS = new OrderedSet<E>();
            tmpS.add(i);
            subsets.add(tmpS);
         }
         ...
         for (int i = 0; i < s.size(); i++) {
            tmpS = new OrderedSet<E>(); // size: Größe: n über k
  ...
  /**
   ...
   public static <E extends Comparable<E>> ArrayList<MathSet<OrderedSet<E>>> partitions(OrderedSet<E> set) {
      ArrayList<MathSet<OrderedSet<E>>> partitions = new ArrayList<MathSet<OrderedSet<E>>>();
      ArrayList<OrderedSet<E>> partition;
      Object[] s = set.toArray();
      ArrayList<int[]> pList = MathSet.partitions(s);
      int n, i;
      for (int[] p : pList) {
         partition = new ArrayList<OrderedSet<E>>();
         n = max(p);
         for (i = 0; i <= n; i++) {
         ...
         partitions.add(new MathSet<OrderedSet<E>>(partition));
      }
      return partitions;
   }
