...
public class Set<E> extends java.util.HashSet<E> {
   ...
   public static <E> ArrayList<Set<E>> subsets(java.util.Set<E> set, int k) {
      ArrayList<Set<E>> subsets = new ArrayList<Set<E>>();
      if (k == 0) {
      ...
      Set<E> tmpS;
      ArrayList<E> s = new ArrayList<E>(set); // represent set as an array list (indexing each element!)
      if (k == 1) {
         for (E i : s) {
            tmpS = new Set<E>();
            tmpS.add(i);
