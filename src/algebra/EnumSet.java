...
public class EnumSet<E extends Enum<E>> {
   ...
   public static <E extends Enum<E>> ArrayList<EnumSet<E>> subsets(EnumSet<E> set, int k) {
      ArrayList<EnumSet<E>> subsets = new ArrayList<EnumSet<E>>();
      if (set.enumSet == null) {
         return subsets;
      }
      if (k == 0) {
         EnumSet<E> tmp = new EnumSet<E>();
...
//System.out.println("### set="+set+", k="+k);
         ...
         for (int i = 0; i < s.size(); i++) {
            ...
            tmpS = new EnumSet<E>(set.enumSet);
...
//System.out.println("### tmp="+tmp+", tmpS="+tmpS+", k="+k);
   ...
   public static <E extends Enum<E>> ArrayList<MathSet<EnumSet<E>>> partitions(EnumSet<E> set) {
      ArrayList<MathSet<EnumSet<E>>> partitions = new ArrayList<MathSet<EnumSet<E>>>();
      MathSet<EnumSet<E>> partition = new MathSet<EnumSet<E>>(), partition_i; // the partitions
      if (set.size() <= 1) {
         partition.add(set);
