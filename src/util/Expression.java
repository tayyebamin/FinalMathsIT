  ...
  /**
	...
	public void setOperators(MathContext defaultMathContext) {
		...
		addFunction(new Function("SIN", 1) {
			...
			public BigDecimal eval(List<BigDecimal> parameters) {
				...
				case DEGREE:
					...
					d = angle.doubleValue();
					//System.out.println("P: " + parameters.get(0).toPlainString() + " angleDEG: " + String.valueOf(angle.doubleValue()) + " Value: " + String.valueOf(d));
				...
				case RADIAN:
					...
					//angle = parameters.get(0).divide(PI.multiply(new BigDecimal(2.0)));
		...
		addFunction(new Function("CBRT", 1) {
			@Override
			public BigDecimal eval(List<BigDecimal> parameters) {
				double d = Math.cbrt(parameters.get(0)
						.doubleValue());
				return new BigDecimal(d, mc);
			}
		});
		...
		addFunction(new Function("COS", 1) {
			...
			public BigDecimal eval(List<BigDecimal> parameters) {
				...
				case RADIAN:
					...
					d = angle.doubleValue();
					//System.out.println("P: " + parameters.get(0).toPlainString() + " angleRAD: " + String.valueOf(angle.doubleValue()) + " Value: " + String.valueOf(d));
					d = Math.cos(d);
					bd = new BigDecimal(d,mc);
					bd = bd.setScale(15,RoundingMode.HALF_UP);
					break;
		...
		addFunction(new Function("ASIN", 1) { // added by av
			...
			public BigDecimal eval(List<BigDecimal> parameters) {
				...
				case DEGREE:
				{
					 d = Math.toDegrees(Math.asin(parameters.get(0)
							.doubleValue()));
					break;
				}
				case RADIAN:
				{
					d = Math.asin(parameters.get(0)
							.doubleValue());
					break;
				}
		...
		addFunction(new Function("ACOS", 1) { // added by av
			...
			public BigDecimal eval(List<BigDecimal> parameters) {
				...
				case DEGREE:
				{
					 d = Math.toDegrees(Math.acos(parameters.get(0)
							.doubleValue()));
					break;
				}
				case RADIAN:
				{
					d = Math.acos(parameters.get(0)
							.doubleValue());
					break;
				}
		...
		addFunction(new Function("ATAN", 1) { // added by av
			...
			public BigDecimal eval(List<BigDecimal> parameters) {
				...
				case DEGREE:
				{
					 d = Math.toDegrees(Math.atan(parameters.get(0)
							.doubleValue()));
					break;
				}
				case RADIAN:
				{
					d = Math.atan(parameters.get(0)
							.doubleValue());
					break;
				}
